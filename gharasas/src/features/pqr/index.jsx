...
const PQRPage = () => {
    ...
    const pqrTypes = [
        ...
        { value: 'Contacto', label: 'Contacto', icon: 'support_agent', description: 'Contáctanos para temas generales' }
    ];
    ...
    const handleTypeSelect = (type) => {
        setFormData({ ...formData, tipoSolicitud: type });
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        
        if (!formData.aceptoHabeasData) {
            Swal.fire({ icon: 'info', title: 'Aviso', text: 'Debes aceptar la Política de Tratamiento de Datos (Ley 1581) para continuar.', confirmButtonColor: '#22c5e8' });
            return;
        }
        ...
        try {
            const radicado = await client.post('/pqr', {
                tipoSolicitud: formData.tipoSolicitud,
                nombreRemitente: formData.nombreRemitente,
                emailRemitente: formData.emailRemitente,
                telefonoRemitente: formData.telefonoRemitente,
                asunto: formData.asunto,
                mensaje: formData.mensaje,
                aceptoHabeasData: formData.aceptoHabeasData,
                turnstileToken: formData.turnstileToken
            });

            if (import.meta.env.DEV) console.log("Radicado exitoso", radicado.data);
            setSubmitStatus('success');
            setFormData({ tipoSolicitud: '', nombreRemitente: '', emailRemitente: '', telefonoRemitente: '', asunto: '', mensaje: '', aceptoHabeasData: false, turnstileToken: '' });
        } catch (error) {
            console.warn('Error connecting to PQR API. Fallback to EmailJS...', error);
            try {
               await emailjs.send(
                   import.meta.env.VITE_EMAILJS_SERVICE_ID_CONTACT || 'service_test',
                   import.meta.env.VITE_EMAILJS_TEMPLATE_ID_FAMILIA || 'template_test',
                   { ...formData },
                   import.meta.env.VITE_EMAILJS_PUBLIC_KEY_CONTACT || 'MOCK_KEY'
               );
               setSubmitStatus('success');
               setFormData({ tipoSolicitud: '', nombreRemitente: '', emailRemitente: '', telefonoRemitente: '', asunto: '', mensaje: '', aceptoHabeasData: false, turnstileToken: '' });
            } catch(emailErr) {
               console.error('EmailJS fallback failed:', emailErr);
            ...
            <section className="container mx-auto px-4 md:px-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {pqrTypes.map((type) => (
                                ...
                                >
                                    <span className={`material-symbols-outlined text-3xl mb-3 block ${formData.tipoSolicitud === type.value
                                        ? 'text-primary dark:text-blue-400'
                                        : 'text-slate-400'
                                        }`}>
                                        {type.icon}
                                    </span>
                                    <span className={`font-bold block mb-1 ${formData.tipoSolicitud === type.value
                                        ? 'text-primary dark:text-blue-400'
                    ...
                    >
                        ...
                        {/* Habeas Data Checkbox & Turnstile */}
                        <div className="mb-6 p-4 rounded-xl border border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-slate-800/50">
                            <label className="flex items-start gap-3 cursor-pointer">
                                <input 
                                    type="checkbox" 
                                    name="aceptoHabeasData" 
                                    checked={formData.aceptoHabeasData} 
                                    onChange={handleInputChange} 
                                    className="mt-1 w-5 h-5 accent-primary dark:accent-cyan-400" 
                                />
                                <span className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                                    * Acepto el <a href="/politica-de-datos" className="text-primary dark:text-blue-400 font-bold hover:underline">Tratamiento de Datos Personales (Ley 1581)</a>. Conozco y apruebo que Ghara SAS almacene mi información para dar respuesta oficial a mi solicitud.
                                </span>
                            </label>
                        ...
                        <motion.button
                            type="submit"
                            disabled={isSending || !formData.tipoSolicitud || !formData.aceptoHabeasData || !formData.turnstileToken}
                        ...
                        <p className="text-center text-xs text-slate-400 mt-4">
                            ...
                            <a href="/politica-de-datos" className="text-primary dark:text-blue-400 hover:underline">
                    ...
                    <div className="grid md:grid-cols-3 gap-6 mt-12">
                        {[
                            ...
                            >
                                <span className="material-symbols-outlined text-3xl text-primary dark:text-blue-400 mb-3 block">{item.icon}</span>
