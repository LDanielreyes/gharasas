...
const Services = () => {
    ...
    const handleSubmitLead = async (e) => {
        ...
        let servicios = [];
        md.getAll('servicio').forEach(val => servicios.push(val));
        
        try {
            await client.post('/leads', {
                nombre: md.get('nombre') || 'Sin Nombre',
                email: md.get('email') || 'no-email@ghara.com',
                telefono: md.get('telefono') || '0000000',
                empresa: '',
                ciudad: md.get('ciudad') || '',
                origen: 'Servicios - Cotización',
                comentarios: `Depto: ${md.get('departamento') || ''}. Fecha preferida: ${md.get('fecha') || ''}. Servicios: ${servicios.join(', ')}`
            });
            Swal.fire({ icon: 'info', title: 'Notificación', text: '¡Cotización enviada correctamente!', confirmButtonColor: '#22c5e8' });
            e.target.reset();
        } catch(err) {
            console.error(err);
