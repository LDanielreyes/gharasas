import React from 'react';
import { NavLink, useNavigate } from 'react-router-dom';
import {
  LayoutDashboard, Package, MessageSquare, HelpCircle,
  AlertCircle, BarChart2, Users, LogOut, X, Headphones, Globe,
} from 'lucide-react';
import Cookies from 'js-cookie';

const GharaLogo = () => (
  <svg viewBox="0 0 425 359" style={{ height: '28px', width: 'auto' }} fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M122.21 136.65L218.91 54.06 332.21 151.12 315.62 165.24 220.68 82.65 122.21 162.76 122.21 136.65z" fill="var(--gh-brand-4)"/>
    <path d="M160.68 152.88L160.68 179.5 217.85 131.71 271.5 177.59 302.85 177.59 218.91 103.47 160.68 152.88z" fill="var(--gh-brand-4)"/>
    <path d="M124.5,181.47v18.7c0,.32-.03.63-.11.94-.26,1.07-1.06,3.27-3.37,3.15-1.77-.09-3.2-1.49-3.52-3.23-1.05-5.76-5.46-22.19-21.58-23.79,0,0-31.24-5.47-35.47,47.47,0,0-1.94,43.06,31.24,42.53,0,0,9.18-.18,13.76-7.41v-25.76s-.81-4.89-8.71-5.27c-.99-.05-1.94-.5-2.53-1.29-.5-.66-.78-1.55-.13-2.6s1.86-1.61,3.09-1.61h28.99c1.03,0,2.06.37,2.74,1.13.83.92,1.17,2.33-1.58,3.99,0,0-3.35,1.94-3.35,8.12v26.47s-8.36,11.09-30.63,11.96c-.61.02-1.21.05-1.82.1-6.18.53-52.01,2.64-53.49-50.19,0,0-.87-28.84,20.86-45.83,7.63-5.96,16.98-9.24,26.63-10,11.13-.88,27.91.26,38.98,12.42Z" fill="var(--gh-brand-4)"/>
    <path d="M133.32,178.65s4.68.18,4.41,6v76.24s.44,4.68-7.41,8.82c0,0-1.94,2.65,1.24,2.65h29.12s4.5,0,1.94-3.23c-.7-.89-7.06-2.41-7.24-8.24v-37.25c0-2.59,1-5.1,2.84-6.92,1.39-1.38,3.37-2.76,6.1-3.54,3.47-.99,7.21-.72,10.53.7,1.23.53,2.52,1.25,3.63,2.26,1.38,1.24,2.13,3.03,2.13,4.88v41.3s-.77,4.75-5.16,5.97c-.93.26-1.75.86-2.12,1.75-.35.84-.31,1.81,1.28,2.34h29.65s3.5-1.68,0-3.76c0,0-4.76-2.24-6.35-7v-43.33c0-4.38-1.64-8.66-4.75-11.75s-8.56-5.91-17.31-5.28c0,0-10.41.53-21.18,11.29v-35.65s.53-5.65-5.47-5.12c0,0-19.47,3.88-15.88,6.88Z" fill="var(--gh-brand-4)"/>
    <path d="M272.72,263.25c-.72-.31-1.53-.38-2.31-.29-1.69.19-5.09.28-4.73-2.26.18-1.24,0-45.18,0-45.18,0,0-1.24-12.88-18.35-14.29,0,0-28.59-4.59-36,14.82,0,0-2.47,10.41,4.76,11.47,0,0,9.18,1.24,10.24-7.41,1.06-8.65,4.59-12.35,11.65-12,0,0,9-.88,10.06,7.59v12.71l-24.71,9.35s-21.18,7.24-14.12,25.76c0,0,4.41,13.94,24.18,8.82,0,0,7.24-2.82,15.35-9.71,0,0,2.47,12.32,10.94,10.83,0,0,5.97.27,13.57-5.34.72-.53,1.23-1.33,1.31-2.22.08-.91-.24-1.98-1.84-2.67ZM248.03,255.59s-11.12,12.53-18.71,6.71c-7.59-5.82-1.06-17.12-1.06-17.12,0,0,4.59-6.53,19.76-10.94v21.35Z" fill="var(--gh-brand-4)"/>
    <path d="M279.07,205.44c-.85.2-1.67.54-2.38,1.05-1.37.99-2.61,2.74,1.52,4.69,0,0,2.1,1.11,4.46,3.08,1.21,1.01,1.89,2.53,1.89,4.11v42.23c0,1.86-.58,3.7-1.76,5.15-.56.69-1.24,1.29-2.03,1.54s-1.48.56-2.01,1.13c-.73.78-1.32,1.91-.23,3,.64.64,1.56.94,2.47.94h27.97c.83,0,1.65-.27,2.3-.8,1-.82,1.9-2.23-.24-3.95-.58-.47-1.29-.73-2.03-.83-2.31-.3-7.13-1.01-7.13-4.84v-38.02c0-1.4.4-2.78,1.18-3.95,1.22-1.83,3.34-4.19,6.03-4.08,1.06.05,2.01.68,2.58,1.57,1.6,2.47,6.2,8.13,12.42,3.53,2.42-1.79,4.23-4.33,4.97-7.24.85-3.31.63-7.95-5.28-11.21-2.8-1.55-6.06-1.98-9.13-1.07-4.07,1.21-9.61,4.48-12.24,13.18v-8.82s-.01-6.71-7.95-4.06l-15.37,3.68Z" fill="var(--gh-brand-4)"/>
    <path d="M395.78,263.25c-.72-.31-1.53-.38-2.31-.29-1.69.19-5.09.28-4.73-2.26.18-1.24,0-45.18,0-45.18,0,0-1.24-12.88-18.35-14.29,0,0-23.52-3.77-33.42,10.01-2.12,2.95-3.29,6.54-2.91,10.16.29,2.74,1.48,5.6,5.1,6.13,0,0,9.18,1.24,10.24-7.41,1.06-8.65,4.59-12.35,11.65-12,0,0,9-.88,10.06,7.59v12.71l-24.71,9.35s-21.18,7.24-14.12,25.76c0,0,3.92,12.38,20.92,9.52,2.2-.37,4.32-1.1,6.31-2.12,2.86-1.47,7.42-4.14,12.31-8.28,0,0,2.47,12.32,10.94,10.83,0,0,5.97.27,13.57-5.34.72-.53,1.23-1.33,1.31-2.22.08-.91-.24-1.98-1.84-2.67ZM371.09,255.59s-11.12,12.53-18.71,6.71c-7.59-5.82-1.06-17.12-1.06-17.12,0,0,4.59-6.53,19.76-10.94v21.35Z" fill="var(--gh-brand-4)"/>
  </svg>
);

const navItems = [
  { name: 'Dashboard',       icon: LayoutDashboard, path: '/' },
  { name: 'Productos',       icon: Package,         path: '/productos' },
  { name: 'PQR',             icon: AlertCircle,     path: '/pqr' },
  { name: 'Reseñas',        icon: MessageSquare,   path: '/resenas' },
  { name: 'Reportes',        icon: BarChart2,       path: '/reportes' },
  { name: 'Metadatos',       icon: Globe,           path: '/seo' },
  { name: 'Q&A / FAQ',      icon: HelpCircle,      path: '/preguntas' },
  { name: 'Administradores', icon: Users,           path: '/usuarios' },
  { name: 'Descargables',    icon: Package,         path: '/descargables' },
];

const Sidebar = ({ isOpen, toggleSidebar }) => {
  const navigate = useNavigate();

  const handleLogout = () => {
    Cookies.remove('accessToken');
    Cookies.remove('refreshToken');
    navigate('/login');
  };

  return (
    <>
      {/* Mobile overlay */}
      {isOpen && (
        <div
          className="fixed inset-0 bg-black/60 z-40 lg:hidden backdrop-blur-sm"
          onClick={toggleSidebar}
        />
      )}

      <aside
        className={`fixed inset-y-0 left-0 z-50 w-[250px] flex flex-col
          transform transition-transform duration-300 ease-in-out
          lg:translate-x-0 shadow-2xl lg:shadow-none
          ${isOpen ? 'translate-x-0' : '-translate-x-full'}
        `}
        style={{
          background: 'var(--gh-surface)',
          borderRight: '1px solid rgba(240,238,232,0.07)'
        }}
      >
        {/* Logo */}
        <div style={{ padding: '22px 20px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <GharaLogo />
            <p style={{
              color: 'var(--gh-text-muted)', fontSize: '0.6rem', textTransform: 'uppercase',
              letterSpacing: '0.14em', marginTop: '5px',
              fontFamily: "'IBM Plex Sans', sans-serif", fontWeight: 500,
            }}>
              Panel Administrativo
            </p>
          </div>
          <button
            onClick={toggleSidebar}
            aria-label="Cerrar menú"
            className="lg:hidden"
            style={{ padding: '4px', color: 'var(--gh-text-muted)', background: 'none', border: 'none', cursor: 'pointer' }}
          >
            <X size={16} />
          </button>
        </div>

        <div style={{ height: '1px', background: 'var(--gh-border)', margin: '0 16px 8px' }} />

        {/* Nav */}
        <nav style={{ flex: 1, padding: '4px 8px', overflowY: 'auto' }}>
          {navItems.map(({ name, icon: Icon, path }) => (
            <NavLink
              key={name}
              to={path}
              end={path === '/'}
              onClick={() => { if (window.innerWidth < 1024) toggleSidebar(); }}
              className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}
            >
              <Icon size={16} strokeWidth={1.75} />
              <span>{name}</span>
            </NavLink>
          ))}
        </nav>

        <div style={{ height: '1px', background: 'var(--gh-border)', margin: '0 16px' }} />

        {/* Support */}
        <div style={{ padding: '12px 16px' }}>
          <button
            style={{
              width: '100%', padding: '9px 12px',
              border: '1px solid rgba(240,238,232,0.08)', borderRadius: '8px',
              color: 'var(--gh-text-muted)', fontSize: '0.78rem', fontWeight: 500,
              display: 'flex', alignItems: 'center', gap: 8,
              background: 'transparent', cursor: 'pointer', transition: 'all 0.15s',
              fontFamily: "'IBM Plex Sans', sans-serif",
            }}
            onMouseEnter={e => { e.currentTarget.style.background = 'var(--gh-border)'; e.currentTarget.style.color = 'var(--gh-text-primary)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--gh-text-muted)'; }}
          >
            <Headphones size={14} strokeWidth={1.75} />
            Soporte Técnico
          </button>
        </div>

        {/* User + Logout */}
        <div style={{ borderTop: '1px solid rgba(240,238,232,0.06)', padding: '14px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '10px' }}>
            <div style={{
              width: 32, height: 32, borderRadius: '8px',
              background: 'rgba(45, 196, 196,0.15)',
              border: '1px solid rgba(45, 196, 196,0.25)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: 'var(--gh-brand-1)', fontWeight: 500, fontSize: '0.8rem',
              fontFamily: "'Syne', sans-serif", flexShrink: 0,
            }}>
              A
            </div>
            <div>
              <p style={{ color: 'var(--gh-text-primary)', fontSize: '0.82rem', fontWeight: 500, lineHeight: 1.2, fontFamily: "'IBM Plex Sans', sans-serif" }}>
                Admin Ghara
              </p>
              <p style={{ color: 'var(--gh-text-muted)', fontSize: '0.68rem', fontFamily: "'IBM Plex Sans', sans-serif" }}>
                System Admin
              </p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            style={{
              display: 'flex', alignItems: 'center', gap: 8,
              width: '100%', padding: '8px 10px',
              color: 'var(--gh-danger)', fontSize: '0.78rem', fontWeight: 500,
              background: 'transparent', border: 'none', cursor: 'pointer',
              borderRadius: '6px', transition: 'background 0.15s',
              fontFamily: "'IBM Plex Sans', sans-serif",
            }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(248,113,113,0.08)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
          >
            <LogOut size={14} strokeWidth={1.75} />
            Cerrar Sesión
          </button>
        </div>
      </aside>

      {isOpen && (
        <div
          style={{ position: 'fixed', inset: 0, zIndex: 40, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(2px)' }}
          className="lg:hidden"
          onClick={toggleSidebar}
        />
      )}
    </>
  );
};

export default Sidebar;
